  ...
  <template>
    <div style="height: calc(100vh - 105px)" >
      ...
      <q-dialog v-model="expanded"  >
        ...
        <dashboardFilter
          :value="filterRecord"
          @update:model-value="(value) => Object.assign(filterRecord, value)"
...
export default {
  ...
  mounted () {
    ...
    if (this.value !== undefined && this.value.registrosSeleccionados && Object.keys(this.value.registrosSeleccionados).length > 0) { // si ya hemos cargado previamente los recargo al volver a este tab
      ...
      this.filterRecord.fechaDesde = date.formatDate(new Date(), 'YYYY-MM-DD')
      this.filterRecord.fechaHasta = date.formatDate(new Date(), 'YYYY-MM-DD')
